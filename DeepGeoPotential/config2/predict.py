...
msa_dim = 5 + 1
ss_dim  = 4 + 1
m_dim,s_dim,z_dim = 32,32,32
N_cycle=3
n_layer=[24,24]
n_head,c=8,16
docheck,block_per_check = True,1
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

def load_model(model_path):
    N_ensemble=1 # not used
    model = DPRmodel.DPR(msa_dim,ss_dim,N_ensemble,N_cycle,m_dim,s_dim,z_dim,n_head,c,n_layer,docheck,block_per_check)
...
def pipeline(fastafile,ssfile,savefile_):
    ...
    target['msa_mask'] = torch.zeros([N_cycle,1,d]).numpy()
